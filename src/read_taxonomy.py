...
class Taxonomy:
    ...
    def read_genome_binning(self, bin_df, convert_to_rsa=False):
        ...
        if convert_to_rsa:
            ...
            df = df.fillna(0)
            df[abundance_cols] = df[abundance_cols].div(
                df[abundance_cols].sum(axis=0), axis=1
            )
        abundace_matrix = df[["bin"] + abundance_cols]

        return abundace_matrix

    def create_taxonomic_abundance_plot(
        self,
        merged_df,
        taxonomic_level,
        output_dir="plots",
        ...
        if len(abundance_cols) == 0:
            print(
                f"No samples found for {layer_filter} layer - skipping {taxonomic_level}"
            )
            return
        ...
        if "Unclassified" in taxa_abundance.index:
            ...
            taxa_abundance = pd.concat(
                [taxa_abundance.drop("Unclassified"), unclassified_row]
            )

        # Assign colors, always set "Unclassified" to grey
        other_taxa = [
            taxon for taxon in taxa_abundance.index if taxon != "Unclassified"
        ]
        ...
        # Customize plot
        ax.set_ylabel(
            "Relative Sequence Abundance (RSA)", fontsize=14, fontweight="bold"
        )
        ax.set_xlabel("Sample", fontsize=14, fontweight="bold")

        # Format x-axis labels
        labels = [
            label.get_text().replace("Depth ", "") for label in ax.get_xticklabels()
        ]
    ...
    def generate_all_plots(self, merged_df, output_dir="plots"):
        ...
        for level in taxonomic_levels:
            self.create_taxonomic_abundance_plot(
                merged_df,
                level,
                output_dir,
                remove_unclassified=False,
...
if __name__ == "__main__":
    ...
    if len(bins_without_tax) > 0:
        missing_abundance = bin_df[bin_df["bin"].isin(bins_without_tax)][
            abundance_cols
        ].sum(axis=0)
