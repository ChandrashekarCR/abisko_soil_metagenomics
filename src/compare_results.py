...
class TaxonomyAnalyzer:
    ...
    def _load_and_process_data(self):
        ...
        self.df["domain"] = self.df["classification"].apply(
            lambda x: self._safe_split(x, 0)
        )
        self.df["phylum"] = self.df["classification"].apply(
            lambda x: self._safe_split(x, 1)
        )
        self.df["class"] = self.df["classification"].apply(
            lambda x: self._safe_split(x, 2)
        )
        self.df["order"] = self.df["classification"].apply(
            lambda x: self._safe_split(x, 3)
        )
        self.df["family"] = self.df["classification"].apply(
            lambda x: self._safe_split(x, 4)
        )
        self.df["genus"] = self.df["classification"].apply(
            lambda x: self._safe_split(x, 5)
        )
        self.df["species"] = self.df["classification"].apply(
            lambda x: self._safe_split(x, 6)
        )
        ...
        for loc in self.locations:
            ...
            for tax_level in self.tax_levels:
                report += self._format_header(
                    f"Taxonomic Level: {tax_level.upper()}", level=2
                )

                top_organisms = set(top_data[tax_level].unique()) - {
                    "Unclassified",
                    "None",
                }
                bottom_organisms = set(bottom_data[tax_level].unique()) - {
                    "Unclassified",
                    "None",
                }
    ...
    def analyze_cross_layer_summary(self):
        ...
        """
        report = self._format_header(
            "PART 2: CROSS-LAYER SUMMARY - GLOBAL LAYER ANALYSIS"
        )
        report += f"\nGenerated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
        ...
        for tax_level in self.tax_levels:
            report += self._format_header(
                f"TAXONOMIC LEVEL: {tax_level.upper()}", level=1
            )

            all_top_organisms = set(
                self.df[self.df["layer"] == "TOP"][tax_level].unique()
            ) - {"Unclassified", "None"}
            all_bottom_organisms = set(
                self.df[self.df["layer"] == "BOTTOM"][tax_level].unique()
            ) - {"Unclassified", "None"}
            ...
            for org in sorted(common_all_layers):
                top_count = len(
                    self.df[(self.df["layer"] == "TOP") & (self.df[tax_level] == org)]
                )
                bottom_count = len(
                    self.df[
    ...
    def analyze_cross_location_comparison(self):
        ...
        for tax_level in self.tax_levels:
            ...
            for loc in self.locations:
                loc_data = self.df[self.df["location"] == loc]
                loc_organisms[loc] = set(loc_data[tax_level].unique()) - {
                    "Unclassified",
                    "None",
                }
            ...
            for i, loc1 in enumerate(self.locations):
                for loc2 in self.locations[i + 1 :]:
                    ...
                    third_loc = [
                        loc for loc in self.locations if loc not in [loc1, loc2]
                    ][0]
                    report += (
                    ...
                    for org in sorted(not_in_third):
                        count1 = len(
                            self.df[
                                (self.df["location"] == loc1)
                                & (self.df[tax_level] == org)
                            ]
                        )
                        count2 = len(
                            self.df[
                                (self.df["location"] == loc2)
                                & (self.df[tax_level] == org)
                            ]
                        )
                    ...
                    summary_data.append(
                        {
                            ...
                            "Unique Phyla": len(
                                set(layer_data["phylum"].unique())
                                - {"Unclassified", "None"}
                            ),
                            "Unique Classes": len(
                                set(layer_data["class"].unique())
                                - {"Unclassified", "None"}
                            ),
                            "Unique Families": len(
                                set(layer_data["family"].unique())
                                - {"Unclassified", "None"}
                            ),
                            "Unique Genera": len(
                                set(layer_data["genus"].unique())
                                - {"Unclassified", "None"}
                            ),
                        }
                    )
    ...
    def generate_master_report(self):
        ...
        """
        master_report = self._format_header(
            "COMPREHENSIVE TAXONOMIC ANALYSIS - MASTER REPORT"
        )
        master_report += (
            f"\nGenerated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
        )
        ...
        master_report += (
            f"Locations: {', '.join(sorted(self.df['location'].unique()))}\n"
        )
        ...
        unclassified_pct = (
            (unclassified_bins / len(self.df)) * 100 if len(self.df) > 0 else 0
        )
        master_report += (
...
Examples:
  ...
  python compare_results.py --help
    ...
    parser.add_argument("input_file", help="Path to the merged taxonomy TSV file")
    parser.add_argument(
        "-o",
        "--output",
